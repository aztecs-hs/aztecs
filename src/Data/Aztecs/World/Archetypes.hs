...
import Data.Aztecs.World.Components (ComponentID, ComponentRef, Components, lookupStorage)
...
getArchetypeComponent :: (Component c) => ComponentID -> ArchetypeComponents -> Maybe (ComponentRef c)
getArchetypeComponent i (ArchetypeComponents m) = do
  d <- Map.lookup i m
  fromDynamic d

insertArchetypeComponent :: (Component c) => ComponentID -> ComponentRef c -> ArchetypeComponents -> ArchetypeComponents
insertArchetypeComponent i r (ArchetypeComponents m) = ArchetypeComponents $ Map.insert i (toDyn r) m

insertArchetypeComponentProxy :: (Component c) => Proxy c -> ComponentID -> ComponentRef c -> ArchetypeComponents -> ArchetypeComponents
  ...
  Nothing ->
    let (es', ids') =
          foldr
                      ...
                      map
                        ( \(e, f) ->
                            let r = (C.componentRef f cId)
                             in (e, insertArchetypeComponentProxy p cId r (ArchetypeComponents mempty))
                        )
                        cs
            ...
            ([], ids)
            (Set.toList a)
     in (ArchetypeId i, Archetypes (IntMap.insert i (ArchetypeState (Archetype a) (Map.fromList es') []) es) ids' as (i + 1))
...
lookupStorageProxy _ = lookupStorage
